...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    const perfectMatches = programMatches.filter(
      (match) => match.matchScore === 100
    );
    const partialMatches = programMatches.filter(
      (match) => match.matchScore < 100 && match.matchScore >= 50
    );
...
export default function ProgramMatches() {
  ...
  const filteredPrograms = useMemo(() => {
    if (!searchQuery) return currentPrograms;

    const query = searchQuery.toLowerCase();
    return currentPrograms.filter(
    ...
    <div className="bg-white shadow rounded-lg p-6">
      ...
      <div className="flex border-b border-gray-200 mb-6">
        ...
        >
          ...
          Recommendations ({partialMatches.length})
        </button>
      </div>
      ...
      <div className="relative mb-6">
        ...
        <input
          type="text"
          placeholder={`Search ${
            activeTab === "perfect" ? "perfect matches" : "recommendations"
          }...`}
          value={searchQuery}
      ...
      {currentPrograms.length === 0 ? (
        <div className="text-center py-12 border border-dashed border-gray-300 rounded-lg">
          ...
          <p className="mt-1 text-sm text-gray-500">
            {activeTab === "perfect"
              ? "Add more qualifications to see perfect matches."
              : "Programs with at least 50% match will appear here as recommendations."}
          </p>
        </div>
        ...
        <div>
          ...
          <div className="mb-4 text-sm text-gray-600">
            ...
            {activeTab === "perfect" ? "perfect match" : "recommendation"}
          ...
          {currentProgram && (
            <ProgramMatchCard
              ...
              isRecommendation={activeTab === "partial"}
            />
          )}
  ...
  return (
    <div
      className={`border rounded-lg p-6 hover:shadow-md transition-shadow ${
        isRecommendation ? "border-yellow-200 bg-yellow-50" : ""
      }`}
    >
      {isRecommendation && (
        <div className="flex items-center mb-3 text-yellow-700 bg-yellow-100 px-3 py-1 rounded-md text-sm font-medium w-fit">
